...
- `X-Hmac: <HMAC-SHA256(api_secret, api_key + nonce + epoch_timestamp_ms + params_hash + body_hash)>`: generates an HMAC (Hash-based Message Authentication Code) to allow validation of authenticity and integrity of the message; encoded as standard base64 string. `+` denotes string concatenation.
